...
//===----------------------------------------------------------------------===//
//                           SwizzleVectorInst Class
//===----------------------------------------------------------------------===//

/// This instruction constructs a dynamic permutation of two input vectors.
///
/// For each element of the result vector, the third input vector selects an
/// element from one of the two input vectors to copy to the result.
/// Non-negative elements in the mask represent an index into the concatenated
/// pair of input vectors. Out-of-bound indexes specify that the result element
/// is poison.
///
/// Scalable vectors are not supported.
class SwizzleVectorInst : public Instruction {
  constexpr static IntrusiveOperandsAllocMarker AllocMarker{3};
...
protected:
  ...
  LLVM_ABI SwizzleVectorInst *cloneImpl() const;

public:
  LLVM_ABI SwizzleVectorInst(Value *V1, Value *V2, Value *V3,
                             const Twine &NameStr = "",
                             InsertPosition InsertBefore = nullptr);
  ...
  /// Return true if a swizzlevector instruction can be
  /// formed with the specified operands.
  LLVM_ABI static bool isValidOperands(const Value *V1, const Value *V2,
                                       const Value *V3);
  ...
  // Methods for support type inquiry through isa, cast, and dyn_cast:
  static bool classof(const Instruction *I) {
    return I->getOpcode() == Instruction::SwizzleVector;
  }
  static bool classof(const Value *V) {
    return isa<Instruction>(V) && classof(cast<Instruction>(V));
  }
...
template <>
struct OperandTraits<SwizzleVectorInst>
    : public FixedNumOperandTraits<SwizzleVectorInst, 3> {};

DEFINE_TRANSPARENT_OPERAND_ACCESSORS(SwizzleVectorInst, Value)
